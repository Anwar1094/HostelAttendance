...
app.post('/save-image', (req, res) => {
    ...
    connection.query(`Update StudentsData Set Image='${imageDataUrl}' where Student_ID='${id}';`, (err, results, fields) => {
        if (err) {
...
app.post('/submitComplain', upload.single('file'), async (req, res) => {
    let complainData = req.body.complainData;
    ...
    if (req.file) {
        ...
        pool.query('Insert into Complaints (Date, Student_ID, complain, Warden_name, Description,filename, filedata) values (?,?,?,?,?,?,?);', complainData, (err, result) => {
        ...
        console.log('r2')
        pool.query('Insert into Complaints (Date, Student_ID, complain, Warden_name, Description) values (?,?,?,?,?);', complainData, (err, result) => {
            console.log(err)
            if (err) return res.status(500).json(err);
            return res.status(200).json({ 'success': true })
        })
